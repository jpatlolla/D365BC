codeunit 70009700 "eShop Functions"
{
    ...
    begin
        FOR i := 1 TO ArraySize DO BEGIN
            IF ItemRec.Get(ItemNumber[i]) then;
            EshopPricing(ItemNumber[i], VariantCode[i], CustomerNumber, ItemRec."Base Unit of Measure", 1, PriceDecimal[i], SalesPriceDecimal[i], "Discount%"[i]);
    ...
    begin
        ...
        END;
        IF ReportIDInt = 10076 THEN BEGIN
            CLEAR(SH);
            SH.SetRange("Document Type", sh."Document Type"::Quote);
            IF Number <> 'NULL' THEN
                SH.SETRANGE("No.", Number)
            ELSE BEGIN
                IF ERPCustomerNumber <> '' THEN
                ...
                IF EndDate <> 'NULL' THEN
                    EVALUATE(EDate, EndDate);
                IF (SDate <> 0D) OR (EDate <> 0D) THEN
                    SH.SETRANGE("Document Date", SDate, EDate);
            END;
            IF SH.FindSet() THEN begin
                Clear(NvRecRef);
                NvRecRef.GetTable(SH);
        ...
        END;
        IF ReportIDInt = 10121 then begin
            Clear(PIH);
            IF Number <> 'NULL' THEN
                PIH.SETRANGE("No.", Number)
            ELSE BEGIN
                IF ERPCustomerNumber <> '' THEN
                    PIH.SETRANGE("Buy-from Vendor No.", ERPCustomerNumber);
                IF StartDate <> 'NULL' THEN
                    EVALUATE(SDate, StartDate);
                IF EndDate <> 'NULL' THEN
                    EVALUATE(EDate, EndDate);
                IF (SDate <> 0D) OR (EDate <> 0D) THEN
                    PIH.SETRANGE("Posting Date", SDate, EDate);
            END;
            IF PIH.FindSet() THEN begin
                Clear(NvRecRef);
                NvRecRef.GetTable(PIH);
                Glotempblob.Blob.CreateOutStream(NvOutStream);
                Report.SaveAs(ReportIDInt, '', ReportFormat::Pdf, NVOutStream, NvRecRef);
                Glotempblob.Insert();
            end;
        END;
        IF ReportIDInt = 10122 then begin
            CLEAR(PH);
            PH.SetRange("Document Type", PH."Document Type"::Order);
            IF Number <> 'NULL' THEN
                PH.SETRANGE("No.", Number)
            ELSE BEGIN
                IF ERPCustomerNumber <> '' THEN
                    PH.SETRANGE("Buy-from Vendor No.", ERPCustomerNumber);
                IF StartDate <> 'NULL' THEN
                    EVALUATE(SDate, StartDate);
                IF EndDate <> 'NULL' THEN
                    EVALUATE(EDate, EndDate);
                IF (SDate <> 0D) OR (EDate <> 0D) THEN
                    PH.SETRANGE("Order Date", SDate, EDate);
            END;
            IF PH.FindSet() THEN begin
                Clear(NvRecRef);
                NvRecRef.GetTable(PH);
                Glotempblob.Blob.CreateOutStream(NvOutStream);
                Report.SaveAs(ReportIDInt, '', ReportFormat::Pdf, NVOutStream, NvRecRef);
                Glotempblob.Insert();
            end;
        END;
        IF ReportIDInt = 10123 then begin
            CLEAR(PH);
            PH.SetRange("Document Type", PH."Document Type"::Quote);
            IF Number <> 'NULL' THEN
                PH.SETRANGE("No.", Number)
            ELSE BEGIN
                IF ERPCustomerNumber <> '' THEN
                ...
                IF EndDate <> 'NULL' THEN
                    EVALUATE(EDate, EndDate);
                IF (SDate <> 0D) OR (EDate <> 0D) THEN
                    PH.SETRANGE("Document Date", SDate, EDate);
            END;
            IF PH.FindSet() THEN begin
                Clear(NvRecRef);
                NvRecRef.GetTable(PH);
    ...
    /*procedure InsertRecordLinks(URL1: Text[1024]; Company: Text[250]; Description: Text[250]; CreatedSpecified: Text[250]; "Document Name": Text[250]; "Record No": Text[250]; IsInvoice: Boolean; LineNo: Integer; VendorInvoiceNo: Code[80]; JobContractEntryNo: Integer; QtyToInvoice: Decimal; VendorInvoiceDate: Date; InvoiceValue: Decimal; VatValue: Decimal; QtyInvcd: Decimal; var LinkID: Integer)
    var
        RecordLinks: Record "Record Link";
        EM: Codeunit "eShop Methods";
        EshopSetup: Record "eShop Setup";
        VendorRec: Record Vendor;
        LinkID1: Integer;
        PurchExtn: Record "Purchase Header";
        VendorNo: Code[30];
    begin
        EshopSetup.GET;
        IF (STRPOS("Record No", 'Order') <> 0) THEN BEGIN
            IF LinkID1 = 0 THEN BEGIN
                PurchExtn.SETRANGE("Document Type", PurchExtn."Document Type"::Order);
                PurchExtn.SETRANGE(PurchExtn."No.", COPYSTR("Record No", STRPOS("Record No", ',') + 1));
                IF PurchExtn.FINDFIRST THEN;
                RecordLinks.INIT;
                //RecordLinks.VALIDATE(Description, Description);
                Evaluate(RecordLinks."Record ID","Record No");
                //RecordLinks.VALIDATE(URL4,"Record No");
                //RecordLinks.VALIDATE("Record No", "Record No");
                RecordLinks.VALIDATE(RecordLinks.URL1, URL1);
                RecordLinks.VALIDATE(Company, COMPANYNAME);
                RecordLinks.VALIDATE(Description, "Document Name");
                //RecordLinks.VALIDATE("Created From Portal", TRUE);
                RecordLinks.INSERT(TRUE);
                LinkID := RecordLinks."Link ID";
            END;
        END;

        IF (STRPOS("Record No", 'Vendor') <> 0) THEN BEGIN
            VendorNo := COPYSTR("Record No", STRPOS("Record No", ':') + 2);
            RecordLinks.INIT;
            RecordLinks.VALIDATE(Description, "Document Name");
            //RecordLinks.VALIDATE("Record No", "Record No");
            Evaluate(RecordLinks."Record ID", "Record No");
            RecordLinks.VALIDATE(RecordLinks.URL1, URL1);
            RecordLinks.VALIDATE(Company, COMPANYNAME);
            //RecordLinks.VALIDATE("Document Name", "Document Name");
            //RecordLinks.VALIDATE("Created From Portal", TRUE);
            RecordLinks.INSERT(TRUE);
            LinkID := RecordLinks."Link ID";
        END;
    end;*/
    procedure GetCustomerBilingDetails(DocumentNo: code[20]; var BillToName: Text[50];
       var BillToAddress: text[50]; var BillToAddress2: text[50]; var BillToCity: text[30];
       var BillToContact: Text[50]; var BillToPostCode: Code[20]; var BillToCounty: Text[30];
       var BillToCountryRegionCode: Code[10]; var BillToEmail: Text[50])
    var
        CLERec: Record 21;
        CustRec: Record 18;
        SalesInvcHeader: Record 112;
    begin

        IF DocumentNo <> '' THEN
            IF SalesInvcHeader.GET(DocumentNo) THEN
                BillToName := SalesInvcHeader."Bill-to Name";
        BillToAddress := SalesInvcHeader."Bill-to Address";
        BillToAddress2 := SalesInvcHeader."Bill-to Address 2";
        BillToCity := SalesInvcHeader."Bill-to City";
        BillToContact := SalesInvcHeader."Bill-to Contact";
        BillToPostCode := SalesInvcHeader."Bill-to Post Code";
        BillToCounty := SalesInvcHeader."Bill-to County";
        BillToCountryRegionCode := SalesInvcHeader."Bill-to Country/Region Code";

        CustRec.SETRANGE("No.", SalesInvcHeader."Bill-to Customer No.");
        IF CustRec.FINDFIRST THEN
            BillToEmail := CustRec."E-Mail";
    end;

    procedure CreateJournal(OrderNo: Code[20]; CustomerNo: Code[20]; Amount: Decimal;
      Default: Boolean; CreditCardNo: Text[20]; HolderName: Text[30]; ExpairyMonth: Text[10];
      ExpairyYear: Text[10]; CVV: Text[20]; VAR ResponseText: Text[1024]);
    var
        JournalLine: Record 81;
        TempJournalLine: Record 81;
        EshopSetup: Record 70009700;
    ...
    begin
        ...
        EshopSetup.GET;
        JournalLine.INIT;
        JournalLine."Journal Template Name" := 'CASHRCPT';
        JournalLine."Journal Batch Name" := 'WEBPAY';
        TempJournalLine.SETRANGE(TempJournalLine."Journal Template Name", 'CASHRCPT');
        TempJournalLine.SETRANGE(TempJournalLine."Journal Batch Name", 'WEBPAY');
        IF TempJournalLine.FINDLAST THEN
            JournalLine."Line No." := TempJournalLine."Line No." + 10000
        ELSE
            JournalLine."Line No." := 1000;

        JournalLine."Posting Date" := TODAY;
        JournalLine."Document Type" := JournalLine."Document Type"::Invoice;
        JournalLine."Document No." := OrderNo;
        JournalLine."Account Type" := JournalLine."Account Type"::Customer;
        JournalLine."Account No." := CustomerNo;
        JournalLine.Description := 'Invoice ' + OrderNo;
        JournalLine.Amount := Amount;
        JournalLine."Bal. Account Type" := JournalLine."Bal. Account Type"::"G/L Account";
        //JournalLine."Bal. Account No." := EshopSetup."Bal.Account";
        JournalLine."Applies-to Doc. Type" := JournalLine."Applies-to Doc. Type"::Invoice;
        JournalLine."Applies-to Doc. No." := OrderNo;
        /*JournalLine."EFT Tender Type" := JournalLine."EFT Tender Type"::"Credit Card";
        JournalLine."EFT Account Number" := CreditCardNo;
        JournalLine."Issuer Identification Number" := CVV;
        JournalLine.VALIDATE("Expiration Month", ExpairyMonth);
        JournalLine.VALIDATE("Expiration Year", ExpairyYear);*/
        IsJournalCreated := JournalLine.INSERT;
        IF IsJournalCreated THEN
            ResponseText := 'Thank you, your payment is being processed. Payments will be reflected on your account the next business day.';
    end;
}
codeunit 70009701 "eShop Methods"
{
    ...
    begin
        if SalesHeader."eShop Shipping Amount" <> 0 then
            ...
            IF NOT SalesLine.GET(SalesHeader."Document Type", SalesHeader."No.", SalesHeader."eShop Shipping Line No") THEN begin
                CLEAR(Salesline1);
